...
export async function migrateContract(
  terra: LocalTerra | LCDClient,
  deployer: Wallet,
  admin: Wallet, // leave this empty then contract is not migratable
  contractAddress: string,
  codeId: number,
  migrate_msg: object
) {
  // new MsgMigrateContract(admin: string, contract: string, new_code_id: number, migrate_msg: object)
  const result = await sendTransaction(terra, deployer, [
